...
/*
 * One licence number per company — but only for the drivers who have one.
 *
 * Most companies leave the licence field blank at first, so a plain unique
 * index would reject the second driver for colliding on "". The exclusion is a
 * partialFilterExpression rather than `sparse`, because the empty values here
 * are empty STRINGS and not missing keys, and sparse only skips missing keys.
 *
 * The two options are also mutually exclusive: MongoDB refuses an index
 * specifying both with "cannot mix partialFilterExpression and sparse", which
 * means an index declared with both is never created at all — the uniqueness it
 * promises silently does not exist. Run scripts/check-indexes.js to see this
 * class of mistake rather than discovering it when two drivers share a licence.
 *
 * The filter is `$gt: ""` rather than the more obvious `$ne: ""` because a
 * partial index accepts only a small set of operators and $ne is not among
 * them. Every non-empty string sorts after the empty one, so the two say the
 * same thing here — and this one can actually be built.
 */
driverSchema.index(
  { companyId: 1, licenceNumber: 1 },
  {
    unique: true,
    partialFilterExpression: { licenceNumber: { $type: "string", $gt: "" } },
  }
);
